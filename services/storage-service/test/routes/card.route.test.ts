...
vi.mock('../../src/repositories/card.repository.js', () => ({ upsertCard: mockUpsert }))
...
describe('POST /card', () => {
  it('retorna 201 e chama upsert e Kafka quando o payload é válido', async () => {
    const response = await app.inject({ method: 'POST', url: '/card', body: validCard })

    expect(response.statusCode).toBe(201)
    expect(mockUpsert).toHaveBeenCalledOnce()
    expect(mockSend).toHaveBeenCalledWith(
      expect.objectContaining({ topic: 'events.card.stored' }),
    )
  })

  it('retorna 400 quando o payload é inválido', async () => {
    const response = await app.inject({ method: 'POST', url: '/card', body: { card_id: 'errado' } })
