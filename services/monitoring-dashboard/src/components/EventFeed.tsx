...
const TOPIC_SUFFIX: Record<string, string> = {
  'events.customer.stored': '→ consolidated',
}

function getKey(event: PipelineEvent): string {
  const d = event.data
  if (d.person_id && event.topic === 'events.customer.stored') return `person #${d.person_id}`
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {events.length === 0 ? (
          ...
          events.map((event, i) => {
            ...
            const suffix = TOPIC_SUFFIX[event.topic]
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#0f1117', borderRadius: 8, borderLeft: `3px solid ${color}`, fontFamily: 'monospace', fontSize: 13 }}>
                <span style={{ color, fontWeight: 700, minWidth: 68, fontSize: 11 }}>{label}</span>
                <span style={{ color: '#94a3b8', flex: 1 }}>
                  {getKey(event)}
                  {suffix && <span style={{ color, opacity: 0.6, marginLeft: 8, fontSize: 11 }}>{suffix}</span>}
                </span>
